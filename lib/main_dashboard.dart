  ...
  class _MainDashboardState extends State<MainDashboard> with RouteAware {
    ...
    int _projectsTabIndex() {
      // Projects tab is 0 for students, 1 for admin/officer/teacher
      if (_userRole == 'user') return 0;
      return 1;
    }
      ...
      Widget trackingPage() {
        ...
        if (projectToShow != null) {
          final screen = ProjectStatusScreen(
            ...
            embedded: true,
            refreshTick: _projectStatusRefresh,            // <-- NEW
        ...
        (_assignLeaderProjectName == null)
            ...
            : AssignLeaderScreen(
                ...
                onAssigned: (assignedUsername) {
                  final full = MockDatabase().getFullNameByUsername(assignedUsername) ?? assignedUsername;
                  if (mounted) {
                    setState(() {
                      _loadUserRoleAndProject();
                      _assignLeaderProjectName = null;          // close the extra
                      _selectedIndex = _indexOfTrackingTab();   // back to Project Status
                      _projectStatusRefresh++;                  // <-- NEW
                    });
                  }
            ...
            : AssignTaskScreen(
                ...
                onCreated: (created) {
                  if (created) {
                    setState(() {
                      _loadUserRoleAndProject();
                      _assignTaskProjectName = null;           // close the extra
                      _selectedIndex = _indexOfTrackingTab();  // back to Project Status
                      _projectStatusRefresh++;                 // <-- NEW
                    });
                  }
